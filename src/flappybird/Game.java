...
public class Game {
    ...
    private boolean highScoresScreenShowing = false;
    private Font flappyFont;
    private int currentThemeIndex = 0;
    private MediaPlayer bgMusicPlayer;
    public boolean isHighScoresScreenShowing() {
        return highScoresScreenShowing;
    }
    public void setHighScoresScreenShowing(boolean showing) {
        this.highScoresScreenShowing = showing;
    }
    public void forceResetHighScoresScreenFlag() {
        this.highScoresScreenShowing = false;
    }
    public static void resetAllHighScoresScreenFlags() {
        Platform.runLater(() -> {
            for (Stage stage : javafx.stage.Window.getWindows().filtered(window -> window instanceof Stage).stream()
                    .map(window -> (Stage) window)
                    .toArray(Stage[]::new)) {
                if (stage.getScene() != null && stage.getScene().getRoot() != null) {
                    resetGameFlagsInNode(stage.getScene().getRoot());
                }
            }
        });
    }
    private static void resetGameFlagsInNode(javafx.scene.Node node) {
    ...
    private void endGame() {
        ...
        if (Settings.getInstance().isFullscreen()) {
            ...
            highScoresButton = new Button("View High Scores");
            highScoresButton.setFont(labelFont);
            ...
            highScoresButton.setLayoutX(455); 
            ...
            highScoresButton.setLayoutX(ScaleHelper.scaleX(226)); 
            highScoresButton.setLayoutY(ScaleHelper.scaleY(280)); 
            highScoresButton.setOnMouseEntered(e -> {
                highScoresButton.setStyle(
                    "-fx-background-color: #FFD700;" + 
                    "-fx-text-fill: #8B4513;" + 
                ...
                highScoresButton.setLayoutX(ScaleHelper.scaleX(228));
                highScoresButton.setLayoutY(ScaleHelper.scaleY(278));
            });
            highScoresButton.setOnMouseExited(e -> {
                highScoresButton.setStyle(
                    "-fx-background-color: gold;" + 
        ...
        root.getChildren().addAll(restartLabel, resetHighScoreLabel, highScoresButton);
        highScoresButton.setOnAction(e -> {
            setHighScoresScreenShowing(true);
            Stage currentStage = (Stage) root.getScene().getWindow();
            Stage highScoresStage = new Stage();
            highScoresStage.initOwner(currentStage);
            highScoresStage.initModality(Modality.APPLICATION_MODAL);
            highScoresStage.setTitle("High Scores");
            highScoresStage.setUserData(this);
            highScoresStage.setOnHidden(event -> {
                setHighScoresScreenShowing(false);
                Platform.runLater(() -> {
            ...
            if (Settings.getInstance().isFullscreen()) {
                highScoresStage.setWidth(1920); 
                highScoresStage.setHeight(1080); 
                highScoresStage.setX(0);
                highScoresStage.setY(0);
                highScoresStage.setFullScreen(true);
                highScoresStage.setFullScreenExitKeyCombination(KeyCombination.NO_MATCH); 
            } else {
                highScoresStage.setWidth(800);
                highScoresStage.setHeight(600);
            }
            highScoresStage.setOnCloseRequest(event -> {
            ...
            Runnable returnToGameOver = () -> {
                this.highScoresScreenShowing = false;
            };
            HighScoresScreen.setOpenedFromGameOver(true);
            new HighScoresScreen(highScoresStage, returnToGameOver).show();
            HighScoresScreen.setOpenedFromGameOver(false);
            highScoresStage.setOnHidden(event -> {
                this.highScoresScreenShowing = false;
            });
        });
    ...
    private void performHighScoreReset() {
        ...
        root.getChildren().add(highScoreLabel);
        saveHighScore();
        String playerName = Settings.getInstance().getPlayerName();
        if (playerName != null && !playerName.isEmpty()) {
            try {
                database.DatabaseManager.getInstance().resetPlayerHighScore(playerName);
            } catch (Exception e) {
            }
        }
    ...
    private void showResetHighScoreConfirmation() {
        ...
        if (Settings.getInstance().isFullscreen()) {
            ...
            dialogFont = Font.font(flappyFont.getFamily(), ScaleHelper.scaleHeight(24));
        }
        ...
        if (Settings.getInstance().isFullscreen()) {
            ...
            buttonFont = Font.font(dialogFont.getFamily(), ScaleHelper.scaleHeight(20)); 
        }
        yesButton.setFont(buttonFont);
        yesButton.setAlignment(Pos.CENTER);
        yesButton.setContentDisplay(javafx.scene.control.ContentDisplay.CENTER);
        double buttonWidth = Settings.getInstance().isFullscreen() ? 220 : ScaleHelper.scaleWidth(120);
        double buttonHeight = Settings.getInstance().isFullscreen() ? 100 : ScaleHelper.scaleHeight(60);
        String buttonStyle = 
            "-fx-background-color: gold;" + 
            "-fx-text-fill: #000000;" + 
            "-fx-font-weight: bold;" +
        ...
        HBox buttonBox = new HBox(40, yesButton, noButton);  
        buttonBox.setAlignment(Pos.CENTER);
        buttonBox.setPadding(new Insets(20));  
        VBox dialogContent = new VBox(30, confirmLabel, buttonBox); 
        ...
        if (Settings.getInstance().isFullscreen()) {
            ...
            dialogWidth = ScaleHelper.scaleWidth(550);
            dialogHeight = ScaleHelper.scaleHeight(320); 
        }
        confirmLabel.setMinWidth(dialogWidth - 60);
        confirmLabel.setMaxWidth(dialogWidth - 60);
        confirmLabel.setPrefWidth(dialogWidth - 60);
    ...
    private void loadHighScore() {
        try {
            String playerName = Settings.getInstance().getPlayerName();
            if (playerName != null && !playerName.isEmpty()) {
                int dbHighScore = database.DatabaseManager.getInstance().getPlayerHighestScore(playerName);
    ...
    private void saveHighScore() {
        try {
            ...
            String playerName = Settings.getInstance().getPlayerName();
            if (playerName != null && !playerName.isEmpty()) {
                database.DatabaseManager.getInstance().saveHighScore(playerName, highScore);
            }
        } catch (Exception e) {
        }
    }
